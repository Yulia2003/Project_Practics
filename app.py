    ...
    while cap.isOpened():
        ...
        if people_count > 0:
            ...
            if last_person_frame >= 0 and frame_count - last_person_frame > gap_threshold:
                interval_duration = frame_count - current_interval_start
                if interval_duration >= min_interval_duration:
...
def video_feed():
    ...
    if not filename:
        logger.error("Filename not provided for video feed")
    ...
    logger.info(f"Starting video stream for: {video_path}")
